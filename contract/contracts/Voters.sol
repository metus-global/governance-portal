...
contract Voters {
  ...
  function loginUser(string memory pass,string memory metus) external view returns (bool){
    require(isUserRegistered());
    User memory tmp=userlist[UserToId[getSender()]];
    bytes32 chkHash=keccak256(abi.encode(pass));
    if(tmp.pass==chkHash && (keccak256(abi.encodePacked(tmp.metus)) == keccak256(abi.encodePacked(metus)))){
      return true;
    }
    else{
      return false;
  ...
  function addUser(string memory metus,string memory name,string memory email,string memory pass,string memory cid,string memory key,bool isValidated) external{
    ...
    userlist.push(User(userID,metus,name,email,hsh,cid,key,isValidated));
